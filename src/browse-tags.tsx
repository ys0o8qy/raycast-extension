...
export default function BrowseTagsCommand() {
  ...
  const tags = Array.from(new Set((data ?? []).flatMap((entry) => entry.tags))).sort((a, b) => a.localeCompare(b));

  return (
    <List isLoading={isLoading} searchBarPlaceholder="Browse tags...">
      {tags.map((tag) => {
        const entries = (data ?? []).filter((entry) => entry.tags.includes(tag));
        return (
          <List.Section key={tag} title={`#${tag}`} subtitle={`${entries.length}`}>
            {entries.map((entry) => (
              <List.Item
                key={entry.id}
                title={entry.title}
